...
// ---------------------------------------------------------
// External MPU Device Descriptor (Required to read sensor)
// ---------------------------------------------------------
...
/**
 * MPU sensor readings JSON handler responds with MPU6500 sensor data
 * @param req HTTP request for which the uri needs to be handled
 * @return ESP_OK
 */
static esp_err_t http_server_get_mpu_sensor_readings_json_handler(httpd_req_t *req)
{
    // ESP_LOGI(TAG, "/mpuSensor.json requested"); // Uncomment for verbose logging

    char mpuSensorJSON[256]; // Increased buffer size for 7 float values
    mpu6050_acceleration_t accel;
    mpu6050_rotation_t gyro;
    float temp;

    // Read latest data from the sensor (using the global descriptor)
    // Note: For a robust app, you might want to read from a global variable 
    // updated by sensor_task to avoid I2C conflicts, but direct read works if I2C driver is thread-safe.
    // Given i2cdev uses mutexes, direct read here is safe!
    mpu6050_get_motion(&sensor_dev, &accel, &gyro);
    mpu6050_get_temperature(&sensor_dev, &temp);

    // Format the JSON string
    sprintf(mpuSensorJSON, 
            "{\"temp\":\"%.1f\",\"accX\":\"%.2f\",\"accY\":\"%.2f\",\"accZ\":\"%.2f\",\"gyrX\":\"%.2f\",\"gyrY\":\"%.2f\",\"gyrZ\":\"%.2f\"}", 
            temp, 
...
static httpd_handle_t http_server_configure(void)
{
    ...
    if (httpd_start(&http_server_handle, &config) == ESP_OK)
    {
        ...
        // register mpuSensor.json handler (REPLACED DHT)
        httpd_uri_t mpu_sensor_json = {
                .uri = "/mpuSensor.json",
                .method = HTTP_GET,
                .handler = http_server_get_mpu_sensor_readings_json_handler,
                .user_ctx = NULL
        };
        httpd_register_uri_handler(http_server_handle, &mpu_sensor_json);
