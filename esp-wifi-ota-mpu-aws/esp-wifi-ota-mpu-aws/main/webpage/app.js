...
/**
 * Gets MPU6500 sensor values for display on the web page.
 */
function getMPUSensorValues()
{
	// We will fetch from a new endpoint called /mpuSensor.json
	$.getJSON('/mpuSensor.json', function(data) {
		// Update Temperature
		$("#temperature_reading").text(data["temp"]);
		
		// Update Accelerometer
		$("#accel_x").text(data["accX"]);
		$("#accel_y").text(data["accY"]);
		$("#accel_z").text(data["accZ"]);

		// Update Gyroscope
		$("#gyro_x").text(data["gyrX"]);
		$("#gyro_y").text(data["gyrY"]);
		$("#gyro_z").text(data["gyrZ"]);
	});
}

/**
 * Sets the interval for getting the updated MPU sensor values.
 */
function startMPUSensorInterval()
{
	// 1000ms (1 second) refresh rate for better responsiveness
